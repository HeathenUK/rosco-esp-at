...
uint16_t process_incoming_esp(State_esp *state) {
	...
	uint16_t count = unbuffer(state->ringBuffer, buffer);      //Pull the ring buffer
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (queryraised == true) {
                fctprintf(mcSendDevice, &duart_a, "Processing following a query being raised\r\n");

                switch(state->state) {
                    
                    case STATE_DISCARD:
                        if (buffer[i] == '\r') {
                            state->state = STATE_AWAIT_PLUS;
                            fctprintf(mcSendDevice, &duart_a, "Start of a \\r, be on the lookout for a +\r\n");
                            printf("%c", buffer[i]);
                        } else {
...
void i2c_read(CharDevice *uart, unsigned int i2caddr, uint8_t reg, uint8_t len) {
    ...
    fctprintf(mcSendDevice, uart, "AT+DRVI2CWRBYTES=0,0x%02x,0x%02x\r", i2caddr, reg);
